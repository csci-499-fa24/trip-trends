...
const EditTripComponent = ({
    ...
    const [tripLocationsState, setTripLocationsState] = useState(
        tripLocations || []
    );
    ...
    const getImageURL = async (trip_location) => {
        try {
            ...
            if (response.response && response.response.results.length > 0) {
                const images = response.response.results.map(
                    (image) => image.urls.regular
                ); // array of 10 images

                const random_index = Math.floor(
                    Math.random() * response.response.results.length
                ); // random index img to set as image url
    ...
    const selectLocation = (suggestion) => {
        if (
            !tripLocationsState.includes(suggestion) &&
            tripLocationsState.length < 10
        ) {
    ...
    const handleEdit = async (e) => {
        ...
        try {
            ...
            const addedLocations = tripLocationsState.filter(
                (location) => !tripLocations.includes(location)
            );
            ...
            const imageURLs = await Promise.all(
                addedLocations.map(async (location) => {
                    ...
                    if (imageURL) {
                        await axios.post(
                            `${process.env.NEXT_PUBLIC_SERVER_URL}/api/images/trips/${tripId}`,
                            { image_url: imageURL }
                        );
                    }
                })
                        ...
                        <form onSubmit={handleEdit}>
                            ...
                            <div>
                                {tripLocationsState.map((location, index) => (
                                    <div
                                        key={index}
                                        className="edit-trip-selected-location"
                                    >
                                        <span className="edit-trip-location-text">
                                            {location}
                                        </span>
                                        <div
                                            className="edit-trip-icon-div"
                                            onClick={() => {
                                                setTripLocationsState((prev) =>
                                                    prev.filter(
                                                        (loc, i) => i !== index
                                                    )
                                                );
                            ...
                            <button type="submit" className="edit-trip-button">
                                Submit
                            </button>
                        </form>
                    </div>
